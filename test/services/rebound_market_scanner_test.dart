...
List<List<dynamic>> _flatKlinesRaw(int count) {
  return List.generate(count, (i) {
    ...
    return <dynamic>[
      t, t + 59999, '100.0', '101.0', '99.0', '100.0', '10.0',
      1000.0, 50, true,
    ];
  });
}
...
List<List<dynamic>> _vReboundRaw() {
  ...
  return bars
      .map((k) => <dynamic>[
            ...
            k.time.millisecondsSinceEpoch + 59999,
            k.open.toString(),
            k.high.toString(),
            k.low.toString(),
            k.close.toString(),
...
/// Mock BinanceApiService：记录所有 getRecentKlines 调用并按规则返回。
class _MockApi extends BinanceApiService {
  ...
  final Map<String, List<List<dynamic>>> fixtures = {};

  /// 需要抛异常的 (symbol, tf) 组合。
  final Set<String> throwOn = {};

  /// 完成所有请求的 Completer — 用于重入保护测试，让首个 scanOnce 保持挂起。
  @override
...
void main() {
  ...
  group('ReboundMarketScanner', () {
    ...
    test('限流预算：所有请求 limit==99，总数 == 标的 × TF', () async {
      ...
      api.fixtures = {for (final s in symbols) s: _flatKlinesRaw(99)};
    ...
    test('错峰分批：每批 ≤ batchSize 个 symbol 并发', () async {
      ...
      int active = 0;
      int maxActive = 0;
      final wrapped = _WrappedApi(api, onEnter: () {
        active++;
    ...
    test('detector 复用：scanner 调 detector.evaluate 且 symbol/timeframe 正确', () async {
      ...
      api.fixtures = {for (final s in symbols) s: _vReboundRaw()};
    ...
    test('重入保护：scanOnce 重叠时第二次立即 no-op，不翻倍 api 调用', () async {
      ...
      expect(api.calls.length, 10 * 4,
          reason: '重入守卫应阻止第二次 scanOnce 翻倍 api 调用');
    });
  });
}

/// 包装 BinanceApiService 用于观察并发度。
class _WrappedApi extends BinanceApiService {
  final _MockApi _inner;
  final void Function() onEnter;
  final void Function() onExit;
  _WrappedApi(this._inner, {required this.onEnter, required this.onExit});
  ...
  Future<List<dynamic>> getRecentKlines({
    ...
    onEnter();
    try {
      return await _inner.getRecentKlines(
...
/// Gated API：让首个请求阻塞直到 release() 被调用。
class _GatedApi extends BinanceApiService {
  final _MockApi _inner;
