...
- [x] 06-01-PLAN.md — 数据模型与数据导入管线（BacktestConfig/Report/Trade 模型 + DataImportService + FundingRateService）

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 06-02-PLAN.md — 回测引擎 + 模拟交易 + Walk-Forward + 报告生成 + Lookahead 测试

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 06-03-PLAN.md — BacktestProvider + BacktestScreen + ProfileScreen 入口 + 集成接线

### Phase 7: 反弹测试调试页面（模拟数据 + 实时检测 + K 线可视化）

**Goal**: 创建一个独立的测试调试页面，用于验证反弹检测逻辑——每 5 秒自动生成模拟分时数据，实时运行 ReboundDetector，并在页面上半部分展示 K 线图、下半部分展示高评分通知。
**Depends on**: Phase 2 (ReboundDetector 纯函数)
**Requirements**: TEST-01, TEST-02, TEST-03, TEST-04
**Success Criteria** (what must be TRUE):

  1. 每 5 秒自动生成一根新的模拟 K 线数据，支持 4 种模式：V 型反弹、死猫反弹、随机游走、持续下跌
  2. 生成的数据实时送入 ReboundDetector.evaluate 运行检测，输出 ReboundSignal
  3. 页面上半部分使用 fl_chart CandlestickChart 展示最近 50 根 K 线，高亮下跌段（红色）和拉回段（绿色）
  4. 页面下半部分展示高评分信号（score ≥ 60），包含评分、死猫风险、共振过滤器、时间戳，最多保留 20 条历史
  5. 控制栏支持开始/暂停、数据模式切换、关键参数调整
...
**Execution Order:**
Phases execute in numeric order: 1 → 2 → 3 → 4|5（并行）→ 6 → 7
...
| 7. 反弹测试调试页面 | 2/2 | Complete   | 2026-06-27 |
